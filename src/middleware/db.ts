...
export class DB {
    ...
    private static setUri():Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            try {
                if (env === 'local' || env === 'test' || env === undefined || env === null) {
                    log.info('Local DB start - NODE_ENV: [%s]', process.env.NODE_ENV);
                    resolve('mongodb://' + config.database.host + '/' + config.database.database);
                } else {
                    log.info('NODE_ENV: [%s]', process.env.NODE_ENV);
                    resolve('mongodb://' + config.database.user + ':' + config.database.password + '@' + config.database.host + '/' + config.database.database);
                }
            } catch (error) {
